...
#endif
{
	...
	static int cs4344_sendbuffer(FAR struct cs4344_dev_s *priv)
	{
		...
		while (priv->inflight < CONFIG_CS4344_INFLIGHT && dq_peek(&priv->pendq) != NULL && !priv->paused)
		{
...
#  endif
			...
			FAR struct ap_buffer_s *apb)
	{
		...
		if (priv->mq.f_inode != NULL)
		{
			...
			ret = file_mq_send(&priv->mq, (FAR const char *)&term_msg,sizeof(term_msg),CONFIG_CS4344_MSG_PRIO);
			if (ret < 0)
			{
				auderr("ERROR: file_mq_send failed: %d\n", ret);
			}
	...
	static void *cs4344_workerthread(pthread_addr_t pvarg)
	{
		...
		while (priv->running || priv->inflight > 0)
		{
			...
			else
			{
				...
				audinfo("cs4344_sendbuffer\n");
				cs4344_sendbuffer(priv);
			}

			audinfo("file_mq_receive\n");
		...
		while ((apb = (FAR struct ap_buffer_s *)dq_remfirst(&priv->pendq)) != NULL)
		{
			...
			audinfo("AUDIO_CALLBACK_DEQUEUE\n");
			/* Send the buffer back up to the previous level. */
#ifdef CONFIG_AUDIO_MULTI_SESSION
			priv->dev.upper(priv->dev.priv, AUDIO_CALLBACK_DEQUEUE, apb, OK, NULL);
#else
