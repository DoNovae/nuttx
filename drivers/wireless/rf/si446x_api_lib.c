 ...
 */
#ifdef __C51__
//HBL #pragma maxargs (13)  /* allow 13 bytes for parameters */
#endif
void si446x_set_property( U8 GROUP, U8 NUM_PROPS, U8 START_PROP, ... )
{
    ...
    while(NUM_PROPS--)
    {
        //HBL260623 Pro2Cmd[cmdIndex] = va_arg (argList, U8);
    	Pro2Cmd[cmdIndex] = va_arg (argList, U32);
        cmdIndex++;
    }
    va_end(argList);
